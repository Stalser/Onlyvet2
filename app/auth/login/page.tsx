...
export default function LoginPage(){
  ...
  async function sendCode(e:React.FormEvent<HTMLFormElement>){
    ...
    if(res.ok){ setSent(true); setMsg(`Код отправлен на email${data.code ? ' (dev-код: '+data.code+')' : ''}`); }
      ...
      <div className="card max-w-lg mx-auto">
        ...
        {!sent ? (
          ...
          <form onSubmit={verify} className="grid gap-4">
            ...
            <div><label className="label">Код</label><input className="input" value={code} onChange={e=>setCode(e.target.value)} required/></div>
