...
export default function Reviews() {
  ...
  const Card: React.FC<{ r: Review }> = ({ r }) => {
    ...
    return h('article', { className: 'bg-white rounded-2xl shadow-soft p-5 flex flex-col h-full' },
      h('div', { className: 'flex items-center justify-between' },
        h('div', { className: 'font-semibold', style: { color: 'var(--navy)' } }, r.name),
        h('div', { className: 'ml-2' }, h(Stars as any, { value: r.rating }))
      ),
      h('div', { className: `mt-3 w-full ${isDesktop ? 'h-24' : 'h-28'} rounded-xl overflow-hidden ring-1 ring-black/5 shadow-sm`, style: { background: 'var(--cloud)' } },
        h('img', {
      ...
      h('p', { className: 'mt-3 text-sm text-gray-800 leading-6' }, preview),
      h('div', { className: 'pt-2' },
        h('button', { className: 'text-teal text-sm hover:underline', onClick: () => setFull(r) }, 'Читать далее')
      )
    );
  };

  const header = h('div', { className: 'mb-6 flex flex-wrap items-center justify-between gap-3' },
    h('div', { className: 'flex items-center gap-3' },
      h('h2', { className: 'text-2xl sm:text-3xl font-bold', style: { color: 'var(--navy)', fontFamily: 'var(--font-montserrat)' } }, 'Отзывы'),
      h('div', { className: 'text-sm text-gray-500 flex items-center gap-1' },
        h('span', { className: 'font-semibold', style: { color: 'var(--navy)' } }, String(avg)),
        h('span', { className: 'text-xs' }, '/ 5'),
        h('span', { className: 'ml-2' }, h(Stars as any, { value: avg })),
        h('span', { className: 'ml-2 opacity-80' }, `· ${items.length}`)
      )
    ),
    h('div', { className: 'flex items-center gap-3' },
      h(Link as any, { href: '/reviews', className: 'btn bg-white border border-gray-300 rounded-xl px-3 sm:px-4' }, `Смотреть все (${items.length})`),
      h('button', { className: 'btn btn-primary', onClick: () => setShowForm(true) }, 'Написать отзыв')
    )
  );

  const desktopGrid = h('div', { className: 'hidden lg:grid grid-cols-3 gap-6' },
    ...items.slice(0, 12).map((r, i) => h(Card, { key: r.id ?? `desk-${i}`, r }))
  );

  const arrows = h('div', { className: 'mb-3 flex items-center gap-2' },
    h('button', {
  ...
  const mobileTrack = h('div', {
    ref: trackRef as any,
    onScroll,
    className: 'no-scrollbar flex gap-3 sm:gap-6 overflow-x-auto snap-x snap-mandatory -mx-2 px-2 sm:mx-0 sm:px-0',
    style: { scrollSnapType: 'x mandatory', scrollbarWidth: 'none' }
  }, ...items.map((r, i) => h('div', { key: r.id ?? `mob-${i}`, className: 'snap-start min-w-[320px] max-w-[320px]' }, h(Card, { r }))));

  const mobile = h('div', { className: 'block lg:hidden' }, arrows, mobileTrack);

  return h('section', { className: 'container py-12 sm:py-16' },
    header,
    desktopGrid,
    mobile,
    full ? h(ReviewFullModal as any, { review: full, onClose: () => setFull(null) }) : null,
    showForm ? h(ReviewModal as any, { onClose: () => setShowForm(false) }) : null
  );
}
