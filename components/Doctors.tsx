      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        {doctors.map((d) => (
          <div key={d.id} className="card">
            ...
            {!!d.allowedServices?.length && (
              <div className="mt-3 flex flex-wrap gap-1.5 sm:gap-2">
                {getServiceChips(d.allowedServices).map((s) => (
                  <span
                    key={s.slug}
                    className="px-2 py-1 rounded-lg border bg-[var(--cloud)] text-[11px] sm:text-xs inline-flex items-center gap-1"
                  >
                    <span>{s.icon ?? '🐾'}</span>
            ...
            <div className="mt-3 sm:mt-4 grid grid-cols-2 gap-2">
              <button className="btn btn-secondary" onClick={() => setDetailsId(d.id)}>
                Подробнее
              </button>
              <button className="btn btn-primary" onClick={() => setScheduleFor(d.id)}>
                Записаться
              </button>
            </div>
          </div>
