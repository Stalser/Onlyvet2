// components/DoctorsHomeBlock.tsx
'use client';

import Link from 'next/link';
import Image from 'next/image';
import { doctors } from '@/lib/data';
import { servicesPricing, doctorServicesMap } from '@/lib/pricing';

const MAX_HOME_DOCTORS = 3;

export default function DoctorsHomeBlock(){
  const list = doctors.slice(0, MAX_HOME_DOCTORS);

  const getMiniItems = (email: string) => {
    const codes = doctorServicesMap[email] || [];
    return servicesPricing.filter((s) => codes.includes(s.code));
  };

  return (
    <section className="container py-12 sm:py-16">
      <div className="flex items-end justify-between gap-3 mb-6 flex-wrap">
        <div>
          <h2 className="text-3xl font-bold" style={{ color: 'var(--navy)' }}>
            Врачи OnlyVet
          </h2>
          <p className="opacity-80 text-sm sm:text-base max-w-xl mt-1">
            Онлайн-консультации проводят практикующие ветеринарные врачи с опытом работы в клинике.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Link
            href="/doctors"
            className="btn bg-white border border-gray-300 rounded-xl px-4 text-sm sm:text-base"
          >
            Все врачи
          </Link>
          <Link
            href="/services"
            className="btn btn-primary rounded-xl px-4 text-sm sm:text-base"
          >
            Услуги и цены
          </Link>
        </div>
      </div>

      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        {list.map((doctor) => {
          const items = getMiniItems(doctor.email);
          return (
            <article
              key={doctor.id}
              className="rounded-2xl border border-gray-200 bg-white p-4 flex flex-col gap-3"
            >
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  {doctor.photo && (
                    <div className="w-14 h-14 rounded-full overflow-hidden bg-[var(--cloud)]">
                      <Image
                        src={doctor.photo}
                        alt={doctor.name}
                        width={56}
                        height={56}
                        className="w-full h-full object-cover"
                      />
                    </div>
                  )}
                  <div>
                    <h3
                      className="font-semibold text-lg"
                      style={{ color: 'var(--navy)' }}
                    >
                      {doctor.name}
                    </h3>
                    <div className="text-sm opacity-80">{doctor.specialty}</div>
                    {doctor.experience && (
                      <div className="text-xs opacity-70">
                        Стаж: {doctor.experience} лет
                      </div>
                    )}
                  </div>
                </div>
              </div>

              {items.length > 0 && (
                <div className="rounded-xl bg-[var(--cloud)]/60 p-3 mt-2">
                  <div
                    className="text-xs font-semibold mb-1"
                    style={{ color: 'var(--navy)' }}
                  >
                    Услуги врача
                  </div>
                  <ul className="text-xs space-y-1">
                    {items.map((s) => (
                      <li key={s.code} className="flex justify-between gap-2">
                        <span className="opacity-80">{s.name}</span>
                        <span className="font-semibold">
                          {s.priceRUB !== undefined
                            ? `${s.priceRUB.toLocaleString('ru-RU')} ₽`
                            : 'уточняется'}
                        </span>
                      </li>
                    ))}
                  </ul>
                </div>
              )}

              <div className="flex justify-end gap-2 mt-2">
                <Link
                  href={`/doctors/${doctor.id}`}
                  className="btn bg-white border border-gray-300 rounded-xl px-3 text-sm"
                >
                  Подробнее
                </Link>
                <Link
                  href={{ pathname: '/booking', query: { doctorEmail: doctor.email } }}
                  className="btn btn-primary rounded-xl px-3 text-sm"
                >
                  Записаться
                </Link>
              </div>
            </article>
          );
        })}
      </div>
    </section>
  );
}
