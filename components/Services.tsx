  ...
  return (
    <section className="container py-16">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-3xl font-bold" style={{ fontFamily: 'var(--font-montserrat)', color: 'var(--navy)' }}>Услуги</h2>
        <Link href="/services" className="btn bg-white border border-gray-300 rounded-xl px-4">Все услуги ({countLabel})</Link>
      </div>
        ...
        {filtered.map(s => (
          <article key={s.slug} className="card flex flex-col">
            <div className="flex items-center gap-3 mb-2">
              <div className="w-10 h-10 rounded-full bg-[var(--cloud)] flex items-center justify-center text-lg">{s.emoji}</div>
