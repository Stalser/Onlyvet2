    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-4 overflow-x-auto">
      <div className="min-w-[720px]">
        <div className="grid" style={{gridTemplateColumns:`120px repeat(7, 1fr)`}}>
          <div></div>
          {week.map(d=>(<div key={d.toISOString()} className="text-sm font-semibold" style={{color:'var(--navy)'}}>{d.toLocaleDateString('ru-RU',{weekday:'short', day:'2-digit', month:'2-digit'})}</div>))}
          {hours.map((h,ri)=>{
